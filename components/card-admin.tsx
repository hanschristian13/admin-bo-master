  ...
  return (
    ...
    >
      <div className='flex items-center justify-between p-4'>
        ...
        <BadgeStatus 
          title={data?.role} 
          styleDotStatus={data?.role.toLowerCase() === 'master' ? 'green' : data?.role.toLowerCase() === 'super agent' ? 'orange' : 'blue'}/>
      </div>
      <div className='bg-neutral-100 border-t border-neutral-200 p-4 flex items-center justify-between gap-x-4 text-neutral-400 capitalize'>
        <Badge variant='outline' className='text-sm font-medium text-neutral-300'>dealer: {data?.dealer}</Badge>
      </div>
    </Card>
  )
}
