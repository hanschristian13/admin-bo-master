...
export const ColumnsPlayerTransaction: ColumnDef<PlayerTransactionType>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize whitespace-nowrap" > {format(row.getValue("date"), 'LLL dd, y')} </div>
    )
  },
  {
    accessorKey: 'game',
    ...
    cell: ({ row }) => <div className="block w-full font-medium" >{row.getValue("game")}</div>
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return <div className="block text-right font-medium">
    ...
    footer: ({ table }) => {
      ...
      return <div className="block w-full text-right font-medium" >
        <span className="text-neutral-300">Rp</span>
        <span className="text-neutral-400">{formatNumberWithCommas(total > 0 ? total : total * -1)}</span>
      </div>;
    },
  },
  {
    ...
    cell: ({ row }) => {
      const dataWin = row.getValue("win") as number
      return <div className="block w-full text-right font-medium" >
        <span className="text-neutral-300">Rp</span>
        <span className="text-neutral-400">{formatNumberWithCommas(dataWin > 0 ? dataWin : dataWin * -1)}</span>
      </div>
    },
    footer: ({ table }) => {
      const total = table.getRowModel().rows.reduce((sum, row) => {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const data = row.getValue("balance") as number
      return <div className="block w-full text-right font-medium" >
        <span className="text-neutral-300">Rp</span>
        <span className={cn(
          data > 0 && "text-green-950",
          data < 0 && "text-red-950",
        )}>{formatNumberWithCommas(data > 0 ? data : data * -1)}</span>
      </div>
    }
  },
]
