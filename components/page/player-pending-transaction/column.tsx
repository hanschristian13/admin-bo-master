...
export const ColumnsPlayerPendingTransaction: ColumnDef<PlayerPendingTransactionType>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize whitespace-nowrap truncate max-w-36" > {(row.getValue("round_id"))} </div>
    )
  },
  {
    accessorKey: "status",
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return <div className="block text-right font-medium">
        ...
        {formatNumberWithCommas(row.getValue("bet"), 0)}
      </div>
    },
    footer: ({ table }) => {
      ...
      return <div className="block w-full text-right font-medium" > 
        <span className="text-neutral-300">Rp</span>
        <span className="text-neutral-400">{formatNumberWithCommas(total > 0 ? total : total * -1, 0)}</span>
      </div>;
    },
  },
  {
    ...
    footer: ({ table }) => {
      ...
      return <div className="block w-full text-right font-medium" > 
        <span className="text-neutral-300">Rp</span>
        <span className={cn(
          total > 0 && "text-green-950",
          total < 0 && "text-red-950",
          )}>{formatNumberWithCommas(total > 0 ? total : total * -1, 0)}</span>
      </div>
    },
  },
]
